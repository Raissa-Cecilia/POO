namespace TrabalhoPOO{
    ...
    public class ProdutoRepository{
        ...
        public void Criar(Produto p) { 
            if (p.Preco <= 0) throw new NegocioException("O preço tem que ser maior que zero!");
            p.Id = _idContador++; _produtos.Add(p); 
        }
        public List<Produto> Listar() => _produtos;
        public void Atualizar(int id, decimal novoPreco){
            var p = _produtos.FirstOrDefault(x => x.Id == id) ?? throw new NegocioException("Produto não foi encontrado!");
            p.Preco = novoPreco;
        }
